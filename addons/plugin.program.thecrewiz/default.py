...
def index():
    ...
    if len(BUILDNAME) > 0:
        ...
        build = '[B][COLOR dodgerblue]Installed:[/COLOR][/B][B][COLOR white]%s (v%s)[/COLOR][/B]' % (BUILDNAME, BUILDVERSION)
        try:
            if version > BUILDVERSION: build = '%s [COLOR red][B][UPDATE v%s][/B][/COLOR]' % (build, version)
            addDir(build,'viewbuild', BUILDNAME)
...
def buildMenu():
    ...
    if bf == False:
        ...
        return
    total, count15, count16, count17, count18, count19, count20, count21, adultcount, hidden = wiz.buildCount()
    third = False; addin = []
    if THIRDPARTY == 'true':
    ...
    if len(match) >= 1:
        if SEPERATE == 'true':
            ...
            if count21 > 0:
                state = '+' if SHOW21 == 'false' else '-'
                # addFile('[B]%s Omega Builds(%s)[/B]' % (state, count21), 'togglesetting',  'show21', themeit=THEME3)
                if SHOW21 == 'true':
                    addFile('[B]%s Omega Builds(%s)[/B]' % (state, count21), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description, color in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
                        if kodiv == 21:
                            menu = createMenu('install', '', name)
                            addDir('[%s] [COLOR %s][B] %s [/B][/COLOR](v%s)' % (float(kodi), color, name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
            if count20 > 0:
